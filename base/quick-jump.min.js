...
var h = preact.h, Component = preact.Component;
...
// -------------------------------------------------------------------------- //
var PageMenuButton = /** @class */ (function (_super) {
    __extends(PageMenuButton, _super);
    function PageMenuButton() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    PageMenuButton.prototype.render = function (props) {
        function onClick(e) {
            e.preventDefault();
            props.onClick();
        }
...
var QuickJump = /** @class */ (function (_super) {
    __extends(QuickJump, _super);
    function QuickJump() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.linkIndex = 0;
        _this.focusPlease = false;
        _this.navigatedByKeyboard = false;
        return _this;
    }
    QuickJump.prototype.componentWillMount = function () {
        var _this = this;
        this.setState({
            searchString: '',
            isVisible: false,
        ...
        loadJSON(this.props.baseUrl + "/doc-index.json", function (data) {
            _this.setState({
                fuse: new Fuse(data, {
                    threshold: 0.25,
                    caseSensitive: true,
            ...
            _this.setState({ failedLoading: true });
        });
        ...
        document.addEventListener('keydown', function (e) {
            if (_this.state.isVisible) {
                if (e.key === 'Escape') {
                    ...
                    e.preventDefault();
                    _this.navigateLinks(-1);
                }
                else if (e.key === 'ArrowDown' || (e.key === 'j' && e.ctrlKey)) {
                    e.preventDefault();
                    _this.navigateLinks(+1);
                }
                else if (e.key === 'Enter' && _this.state.activeLinkIndex >= 0) {
                    _this.followActiveLink();
                }
            }
    ...
    QuickJump.prototype.navigateLinks = function (change) {
    ...
    QuickJump.prototype.followActiveLink = function () {
        if (!this.activeLinkAction) {
            return;
        }
        this.activeLinkAction();
    };
    QuickJump.prototype.updateResults = function () {
    ...
    QuickJump.prototype.componentDidUpdate = function () {
    ...
    QuickJump.prototype.componentDidMount = function () {
        this.props.showHideTrigger(this.toggleVisibility.bind(this));
    };
    QuickJump.prototype.render = function (props, state) {
        var _this = this;
        if (state.failedLoading) {
            var usingFileProtocol = window.location.protocol == 'file:';
            return h("div", { id: "search", class: state.isVisible ? '' : 'hidden' },
                h("div", { id: "search-results" },
                    h("p", { class: "error" }, "Failed to load file 'doc-index.json' containing definitions in this package."),
                    usingFileProtocol ? h("p", { class: "error" },
                        "To use quick jump, load this page with HTTP (from a local static file web server) instead of using the ",
                        h("code", null, "file://"),
                        " protocol. (For security reasons, it is not possible to fetch auxiliary files using JS in a HTML page opened with ",
                        h("code", null, "file://"),
                        ".)") : []));
        }
        this.linkIndex = 0;
        ...
        var onMouseOver = function (e) {
            ...
            while (target && typeof target.getAttribute === 'function') {
                var linkIndexString = target.getAttribute('data-link-index');
                if (typeof linkIndexString == 'string') {
                    var linkIndex = parseInt(linkIndexString, 10);
                    _this.setState({ activeLinkIndex: linkIndex });
                    break;
                }
                target = target.parentNode;
            }
        };
        var items = take(10, state.moduleResults).map(function (r) { return _this.renderResultsInModule(r); });
        return h("div", { id: "search", class: state.isVisible ? '' : 'hidden' },
            h("div", { id: "search-form", onMouseDown: stopPropagation },
                h("input", { placeholder: "Search in package by name", ref: function (input) { _this.input = input; }, onFocus: this.show.bind(this), onClick: this.show.bind(this), onInput: this.updateResults.bind(this) })),
            h("div", { id: "search-results", ref: function (el) { _this.searchResults = el; }, onMouseDown: stopPropagation, onMouseOver: onMouseOver }, state.searchString === ''
                ? [h(IntroMsg, null), h(KeyboardShortcuts, null)]
                : items.length == 0
                    ? h(NoResultsMsg, { searchString: state.searchString })
                    : h("ul", null, items)));
    };
    QuickJump.prototype.renderResultsInModule = function (resultsInModule) {
        var _this = this;
        ...
        var expand = function () {
            var newExpanded = Object.assign({}, _this.state.expanded);
            newExpanded[moduleName] = true;
            _this.setState({ expanded: newExpanded });
        };
        var renderItem = function (item) {
            return h("li", { class: "search-result" }, _this.navigationLink(_this.props.baseUrl + "/" + item.link, {}, h(DocHtml, { html: item.display_html })));
        };
        return h("li", { class: "search-module" },
            ...
            h("ul", null,
                ...
                showAll
                    ? []
                    : h("li", { class: "more-results" }, this.actionLink(expand, {}, "show " + (items.length - visibleItems.length) + " more results from this module"))));
    };
    QuickJump.prototype.navigationLink = function (href, attrs) {
        var _this = this;
        var children = [];
        ...
        var action = function () { window.location.href = href; _this.hide(); };
        return this.menuLink.apply(this, [fullAttrs, action].concat(children));
    };
    QuickJump.prototype.actionLink = function (callback, attrs) {
        var children = [];
        for (var _i = 2; _i < arguments.length; _i++) {
            children[_i - 2] = arguments[_i];
        }
        ...
        return this.menuLink.apply(this, [fullAttrs, callback].concat(children));
    };
    QuickJump.prototype.menuLink = function (attrs, action) {
        var _this = this;
        var children = [];
        for (var _i = 2; _i < arguments.length; _i++) {
            children[_i - 2] = arguments[_i];
        ...
        if (linkIndex === this.state.activeLinkIndex) {
            ...
            attrs.ref = function (link) { if (link)
                _this.activeLink = link; };
            this.activeLinkAction = action;
        }
        var newAttrs = Object.assign({ 'data-link-index': linkIndex }, attrs);
        this.linkIndex += 1;
        return h.apply(void 0, ['a', newAttrs].concat(children));
    };
    return QuickJump;
}(Component));
var DocHtml = /** @class */ (function (_super) {
    __extends(DocHtml, _super);
    function DocHtml() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    DocHtml.prototype.shouldComponentUpdate = function (newProps) {
        return this.props.html !== newProps.html;
    };
    DocHtml.prototype.render = function (props) {
...
function IntroMsg() {
    return h("p", null, "You can find any exported type, constructor, class, function or pattern defined in this package by (approximate) name.");
}
function NoResultsMsg(props) {
    var messages = [
        h("p", null,
            "Your search for '",
            props.searchString,
            "' produced the following list of results: ",
            h("code", null, "[]"),
            "."),
        h("p", null,
            h("code", null, "Nothing"),
        ...
        h("p", null,
            h("code", null,
                "Left \"no matches for '",
                props.searchString,
                "'\" :: Either String (NonEmpty SearchResult)"))
    ];
    return messages[(props.searchString || 'a').charCodeAt(0) % messages.length];
}
function init(docBaseUrl, showHide) {
    preact.render(h(QuickJump, { baseUrl: docBaseUrl || ".", showHideTrigger: showHide || addSearchPageMenuButton }), document.body);
}
exports.init = init;
// export to global object
window.quickNav = { init: init };

},{"fuse.js":2,"preact":3}],2:[function(require,module,exports){
/*!
